...
def pytest_addoption(parser):
    ...
    group.addoption('--exhaustive-sort', action='store_true', dest='exhaustive_sort',
            help='For exhaustive sorting the testcases, default is False')
...
def custom_exhaustive_sort(items):
    ...
    while copy_of_items:
        ...
        if param:
            ...
            sorted_items.append(copy_of_items[0])
            copy_of_items.remove(copy_of_items[0])
...
def pytest_collection_modifyitems(session, config, items):
    ...
    if config.option.selective_test_file:
        ...
        log.info("No selective testcases file found, therefore executing all testcases in the file")
    ...
    if config.option.exhaustive_sort:
        ...
        if len(sorted_items) == len(items):
            log.info("Exhaustive sorting of TCs successful !! TCs will be executed in the exhaustive order")
            items[:] = sorted_items
        else:
            log.warning("Exhaustive sorting of TCs failed, TCs will be executed in the order generated by pytest")
    if items:
